...
class Course {
  ...
  constructor(name: string, code: string, department: Department) {
    this.name = name;
    this.code = code;
...
class Student {
  ...
  enterResult(courseCode: string, grade: string): boolean {
    this.courses.forEach((course) => {
      if (course.code === courseCode) {
        let result1: Result = { course, grade };
  ...
  await inquirer
    .prompt([
      ...
      },
      {
        ...
        name: "deptCode",
        message:
          "Enter code or short name for this department (like cs for Computer Science): ",
      },
    ])
    .then((answer) => {
  ...
  await inquirer
    ...
    .then((answer) => {
      ...
      if (typeof debt1 !== "string") {
        ...
        console.log(
          student.name + "'s registration/roll number is: " + student.rollNumber
        );
        console.log(student.name + "'s email is: " + student.email);
        console.log(
          student.name + "'s password for LMS login is: " + student.password
        );
      }
    });
}
async function rmvStdQuests() {
  await inquirer
    ...
    .then((answer) => {
      ...
      if (!flag1) {
        console.log(
          "No student found with roll number(" + answer.rollNo + ")...!"
        );
      }
    });
}
...
async function crsResultQuests(i: number) {
...
async function enrollCrsQuests(student: Student) {
  await inquirer
    .prompt({
      type: "list",
      name: "course",
    ...
    .then((answer) => {
      let courseCode: string = answer.course.split(",")[1];
...
async function driveAdminMenu(answers:any) {
...
async function studentMenu(student: Student) {
  await inquirer
    .prompt({
      type: "list",
      name: "menu",
...
async function login() {
  await inquirer
    ...
    .then(async (answers: any) => {
      if (
        answers.username === Admin.username
